...
<body style="background-image: url('imanges/18544.jpg');background-position: center;background-size: cover;background-repeat: no-repeat;">
                    ...
                    <div class="row">
                        ...
                        <div class="col-3">
                            <select name="subject" id="subject" class="form-control" style="border-radius: 20px;border: 2px solid #DC3545;">
                                <?php
                                    $query = "SELECT * FROM `detailinfo` WHERE code = 'MULTIMIDIA'";
  ...
  $(document).ready(function() {
    // Initial load of data
        ...
        $('#multimidiaForm').on('submit', function(event) {
        ...
        $('#multimidiaForm input, #multimidiaForm select').on('change', function() {
            // Get form values
            var title = $('#title').val();
            var subject = $('#subject').val();
            var createtime = $('#createtime').val();
            ...
            success: function(data) {
                ...
                data.forEach(function(item) {
                    $('#cards-container').append(`
                        <div class="col-3">
                            <div style="width: auto;height: 340px;border-radius: 10px;position: relative;padding: 10px;background-color: white;margin-top: 10px;">
                                <video style="width: 100%; height: 200px;" controls>
                                    <source src="multimidia/${item.path}" type="video/mp4">
                                    Your browser does not support the video tag.
                                </video>
                                <p>${item.title}</p>
                                 <a style="position: absolute;bottom: 5px;width: 92%" target="_blank" href="multimidia/${item.path}">
