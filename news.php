...
<head>
    ...
    <title><?php echo  $detailthai;?></title>
...
<body style="background-image: url('imanges/18544.jpg');background-position: center;background-size: cover;background-repeat: no-repeat;">
            ...
            <div class="col-12">
                ...
                <form id="newsForm">
                    <div class="row">
                        ...
                        <input name="subject_code" type="hidden"  value="<?php echo $subject_id ;?>">
                        <div class="col-3">
                            <select name="subject" id="subject" class="form-control" style="border-radius: 20px;border: 2px solid #DC3545;">
                                <?php
                                    $subject_id = $_GET['subject'];
                                    $query = "SELECT * FROM `detailinfo` WHERE trim(code) like '%NEWS%'";
                                    $result = $con->query($query);
                                    if (!$result) {
                                        die("Query failed: " . $con->error);
                                    }
                                    while ($row = $result->fetch_assoc()) {
                                        ...
                                        if($subject_id ==  $subcode){
                                            echo "<option value='$subcode' selected>$detailthai</option>";
                                        }else{
  ...
  $(document).ready(function() {
    // Initial load of data
        ...
        $('#newsForm input, #newsForm select').on('change', function() {
            // Get form values
            var title = $('#title').val();
            var subject = $('#subject').val();
            var createtime = $('#createtime').val();
            ...
            success: function(data) {
                ...
                data.forEach(function(item) {
                    $('#cards-container').append(`
                          ...
                          <div class="col-3">
                            <a style="text-decoration: none;color: black" href="news-detail.php?id=${item.newdetailid}">
                            <div class="news" style="width: auto;height: 270px;border-radius: 10px;position: relative;padding: 10px;background-color: white;margin-top: 10px;">
                                <div class="top">                        
                                    <img style="width: 100%;height: 150px;" src="news/${item.path}" alt="">
                                </div> 
                                <div class="topic">
                                    <p>${item.title}</p>
                                </div>                       
